...
| 20 | Duplicate workload operation | The fixed-primary continuous counter accepts two fresh live client writes only after separate primary/replica durable receipts and returns an exact retry without another append. The lifecycle successor still confirms a different fixture-preseeded operation after promotion | COMPONENT SOURCE; CONTINUOUS RPO0 PROCESS SOURCE; LIFECYCLE PROCESS SOURCE — live continuous writes are not yet authority-bound or recovered through lifecycle failover |
