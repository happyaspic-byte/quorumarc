...
| 2 | Active process `SIGKILL` | The controller opens one Node A test effect, observes the killed process through repeated signed probes, waits to the policy-derived Epoch-2 bound (currently logical 2250 ms), and automatically promotes/effects Node B through the durable Witness path. A two-host private-LAN campaign repeats this with Node A on 172.30.1.22 and Node B on 172.30.1.21 while Witness remains on Node A | MODEL; WITNESS PROCESS SOURCE; LIFECYCLE PROCESS SOURCE; AUTOMATIC EXECUTOR SOURCE; TWO-HOST-SHARED-WITNESS-LAB — monotonic controller time and test sink; Witness shares Node A's host |
...
| 6 | A/B network partition | A one-shot candidate reaches the Witness but its authenticated A/B replication frame is dropped; no write is acknowledged, no vote is requested, and no effect opens. The separate fixed-primary continuous counter slice also proves peer loss after readiness returns `Unknown`, leaves no replica WAL, and emits no ACK | MODEL; THREE-PROCESS LAB; CONTINUOUS RPO0 PROCESS SOURCE — continuous slice is not lifecycle authority or physical partition evidence |
...
| 10 | Message delay, duplication, and reordering | A bounded proxy injects delay, duplicate delivery, reply loss with exact retry, corruption, and a node-facing two-connection reorder that exchanges the second opaque request before the first | WITNESS PROCESS SOURCE; LIFECYCLE FAULT-PROXY SOURCE — continuous data-path queues remain open |
| 11 | Candidate data lag | A candidate whose recovered WAL is not the exact configured commit/root pair is refused before readiness and before any authority journal is written | COMPONENT SOURCE; MODEL; LIFECYCLE PROCESS SOURCE |
| 12 | Old PromotionProof replay | The active lifecycle node re-evaluates its retained signed envelope against durable accepted authority and refuses replay. After same-journal restart the node returns Standby with a Closed gate, a higher incarnation, the same promotion digest, and no activation receipt; equal-epoch promote is refused as durability conflict | COMPONENT SOURCE; LIFECYCLE PROCESS SOURCE — exact signed-envelope revalidation across restart remains NOT INTEGRATED because the journal stores a digest, not the envelope bytes |
| 13 | Old vote replay | After A epoch 1 and B epoch 2, A's proxy substitutes its correctly signed obsolete epoch-1 Witness request during an epoch-3 attempt; response binding fails and A self-fences. A separate production-verifier test transplants a previously issued Witness vote into a later candidate-signed final certificate and is refused as an invalid Witness signature | COMPONENT SOURCE; WITNESS PROCESS SOURCE; LIFECYCLE FAULT-PROXY SOURCE — injecting a transplanted final certificate into the live promotion path remains open |
...
| 20 | Duplicate workload operation | The fixed-primary continuous counter accepts two fresh live client writes only after separate primary/replica durable receipts and returns an exact retry without another append. After continuous processes release the exact WALs, lifecycle A/Witness bind commit 2/root into epoch 1; after A loss, B binds the same pair into epoch 2 and confirms the exact live head operation shape without another append | COMPONENT SOURCE; CONTINUOUS-LIFECYCLE PROCESS SOURCE — shared-host sequential handoff, not continuous writes during failover or physical RPO-0 |
| 21 | State-root mismatch | A valid WAL whose root differs from the configured exact contract is refused before readiness and before any authority journal is written | COMPONENT SOURCE; LIFECYCLE PROCESS SOURCE |
